...
## Data Collection
- We want to base our synthetic data on the Iris Dataset while still having the flexibility of controllling hardness etc. 

### Analysis Iris Dataset
We want to generate our synthetic data based on the Iris dataset, commonly used for machine learning experiments, so that it still has a connection to a real world problem, despite being synthetic.
The purpose of generating this data instead of using the Iris dataset itself is so that we can be in control of the hardness of the classification problem and the amount of noise present.

The Iris dataset describes flower samples taken from 3 different species: setosa, versicolor and virginica. For each sample, 4 features are measured: sepal length and width, and petal length and width.
In order for our synthetic data to simulate Iris samples, we first need to perform a preliminary study on the feature means and covariances for each of the 3 flower species.

These statistics are calculated and displayed in the following code chunk:
...
- Explain parameters
  - How hardness controls data
  - How noise controls data
  - How number of training samples controls
...
train_ga <- function(train_data, size=10, maxit=200, pcrossover, pmutation, popSize){
    ...
    for (pcrossover in pcrossover_values) {
      for (pmutation in pmutation_values){
        for (popSize in popSize_values) {
            ...
            model <- train_ga(train_data=train_data, size, maxit, pcrossover=pcrossover, pmutation=pmutation, popSize=popSize)
...
train_es <- function(train_data, size, maxit, sigma = 0.5, popSize = 50){

    min_size <- 1
    max_size <- size
    # Based on hyperparameters from hyperparameter search
    sigma <- sigma
    popSize <- popSize
...
hyperparam_search_es <- function(train_data, validation_data, sigma_values, popSize_values) {
    ...
    for (sigma in sigma_values) {
        for (popSize in popSize_values) {
            ...
            model <- train_es(train_data, size, maxit, sigma, popSize)
